...
pub const Package = struct {
    ...
    pub fn getDisplayInfo(self: *const Package, allocator: std.mem.Allocator) ![]const u8 {
        ...
        return std.fmt.allocPrint(allocator, "{s} {s} v{s} - {s}", .{ self.source.getIcon(), self.name, version_str, desc_str });
    }
...
pub const UniversalPackageBrowser = struct {
    ...
    filtered_packages: ArrayList(usize), // indices into packages
    repositories: ArrayList(Repository),

    // Search and filtering
    search_options: SearchOptions,
    search_input: ArrayList(u8),
    ...
    pub fn init(allocator: std.mem.Allocator) !*UniversalPackageBrowser {
        ...
        browser.* = UniversalPackageBrowser{
            ...
            .allocator = allocator,
            .packages = ArrayList(Package).init(allocator),
    ...
    fn render(widget: *Widget, buffer: *Buffer, area: Rect) void {
        ...
        if (y < area.y + area.height) {
            ...
            const stats = std.fmt.allocPrint(self.allocator, "  ({d} packages, {d} repos)", .{ self.filtered_packages.items.len, self.repositories.items.len }) catch return;
    ...
    fn renderPackageDetails(self: *UniversalPackageBrowser, buffer: *Buffer, area: Rect) void {
        if (self.getSelectedPackage()) |pkg| {
            ...
            // Package header
            const header = std.fmt.allocPrint(self.allocator, "{s} {s} {s}", .{ pkg.source.getIcon(), pkg.name, pkg.version orelse "latest" }) catch return;
