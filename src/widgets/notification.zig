...
pub const NotificationType = enum {
    ...
    pub fn componentKey(self: NotificationType) []const u8 {
        return switch (self) {
            .info => "notice.info",
    ...
    pub fn getDefaultStyle(self: NotificationType) Style {
        if (theme_mod.ThemeManager.global()) |manager| {
            const theme = manager.getActiveTheme();
            const key = self.componentKey();

            if (key.len != 0) {
                if (theme.resolveComponentStyle(key)) |component_style| {
                    return component_style;
                }
            }

            var themed = Style.default();
            themed.bg = switch (self) {
                .info => theme.colors.info,
                .success => theme.colors.success,
                .warning => theme.colors.warning,
                .danger => theme.colors.error_color,
                .custom => theme.getPaletteColor("surface") orelse theme.colors.background_panel,
            };

            themed.fg = if (self == .custom)
                theme.colors.text
            else
                theme.colors.background;

            if (key.len != 0) {
                if (theme.getComponentTypography(key)) |preset| {
                    themed.attributes = preset.attributes;
                }
            }
        ...
        return switch (self) {
            ...
            .danger => Style.default().withFg(style.Color.bright_red).withBg(style.Color.red),
...
pub const Notification = struct {
    ...
    pub fn init(id: u32, title: []const u8, message: []const u8, notification_type: NotificationType) Notification {
        ...
        var title_style = default_style;
        title_style.attributes = default_style.attributes;
        title_style.attributes.bold = true;

        if (theme_mod.ThemeManager.global()) |manager| {
            const theme = manager.getActiveTheme();
            const key = notification_type.componentKey();
            if (key.len != 0) {
                if (theme.getComponentTypography(key)) |preset| {
                    title_style.attributes = preset.attributes;
                    title_style.attributes.bold = true;
                }
            }
        }

        var border_style = default_style;
        border_style.attributes = default_style.attributes;
    ...
    pub fn getProgressPercentage(self: *const Notification) f32 {
        ...
        const elapsed = @as(f32, @floatFromInt(self.timer.read() / std.time.ns_per_ms));
...
pub const NotificationSystem = struct {
    ...
    pub fn showError(self: *NotificationSystem, title: []const u8, message: []const u8) !u32 {
        return self.show(title, message, .danger);
    }
...
pub const NotificationWidget = struct {
    ...
    fn renderNotification(self: *NotificationWidget, buffer: *Buffer, notification: Notification, y: u16) u16 {
        ...
        const notification_area = Rect.init(notification_x, y, notification.width, notification_height);
        ...
        const content_area = Rect.init(notification_area.x + 1, notification_area.y + 1, notification_area.width - 2, notification_area.height - 2);
