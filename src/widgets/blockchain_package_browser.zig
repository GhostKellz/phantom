...
pub const BlockchainPackage = struct {
    ...
    networks: ArrayList(BlockchainNetwork),
    ...
    pub fn getDisplayInfo(self: *const BlockchainPackage, allocator: std.mem.Allocator) ![]const u8 {
        ...
        return std.fmt.allocPrint(allocator, "{s} {s} {s} v{s} - {s} {s}", .{ self.category.getIcon(), security_icon, self.name, version_str, desc_str, self.language });
    }
...
pub const BlockchainPackageBrowser = struct {
    ...
    // Package data
    packages: ArrayList(BlockchainPackage),
    filtered_packages: ArrayList(usize),
    ...
    fn loadBlockchainPackages(self: *BlockchainPackageBrowser) !void {
        ...
        try self.addPackage(BlockchainPackage{
            ...
            .category = .consensus,
            .networks = blk: {
                var networks = ArrayList(BlockchainNetwork).init(self.allocator);
            ...
            .is_audited = true,
            .security_score = 85,
            .dependencies = ArrayList([]const u8).init(self.allocator),
        });
    ...
    fn render(widget: *Widget, buffer: *Buffer, area: Rect) void {
        ...
        if (y < area.y + area.height) {
            ...
            // Show stats
            const stats = std.fmt.allocPrint(self.allocator, "  ({d} packages)", .{self.filtered_packages.items.len}) catch return;
    ...
    fn renderPackageLine(self: *BlockchainPackageBrowser, buffer: *Buffer, x: u16, y: u16, width: u16, pkg: *const BlockchainPackage, is_selected: bool) void {
        ...
        // Networks
        var network_text = ArrayList(u8).init(self.allocator);
    ...
    fn renderPackageDetails(self: *BlockchainPackageBrowser, buffer: *Buffer, area: Rect) void {
        if (self.getSelectedPackage()) |pkg| {
            ...
            // Package header
            const header = std.fmt.allocPrint(self.allocator, "{s} {s} v{s}", .{ pkg.category.getIcon(), pkg.name, pkg.version orelse "latest" }) catch return;
            ...
            const security_info = std.fmt.allocPrint(self.allocator, "Security: {s} (Score: {d}/100)", .{ pkg.getSecurityRating(), pkg.security_score }) catch return;
            ...
            for (pkg.networks.items) |network| {
                const network_info = std.fmt.allocPrint(self.allocator, "  {s} {s}", .{ network.getIcon(), network.getDisplayName() }) catch continue;
    ...
    fn renderNetworkFilter(self: *BlockchainPackageBrowser, buffer: *Buffer, area: Rect) void {
        ...
        for (networks) |network| {
            ...
            const line = std.fmt.allocPrint(self.allocator, "{s} {s}", .{ network.getIcon(), network.getDisplayName() }) catch continue;
            defer self.allocator.free(line);
