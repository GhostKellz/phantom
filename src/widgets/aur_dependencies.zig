...
pub const AURDependencies = struct {
    ...
    dependency_tree: std.ArrayList(Package), // For recursive dependencies
    ...
    expanded_nodes: std.ArrayList(bool), // For tree view
    scroll_offset: usize = 0,
    
    // Search and filtering
    search_query: std.ArrayList(u8),
    filtered_dependencies: std.ArrayList(usize), // indices
    ...
    fn renderPackageInfo(self: *AURDependencies, buffer: *Buffer, x: u16, y: u16, width: u16, package: *const Package) void {
        ...
        const pkg_info = std.fmt.allocPrint(self.allocator, "{s} {s} v{s} ({s})", .{ 
            package.status.getEmoji(), package.name, package.version, package.repo 
        }) catch return;
